...
class datamanager_mnist(object):
    def __init__(self, datapath, train_ratio=None, fold_k=None, norm=True, expand_dim=False, seed=233):
        self.seed = seed
        mnist = np.load(datapath)
...
# mnist = datamanager_mnist(datapath="/home/scw4750/songbinxu/datasets/mnist/mnist.npz", train_ratio=0.8, fold_k=None, expand_dim=True, norm=True, seed=0)
# # colorback_mnist = datamanager_mnist(datapath="/home/scw4750/songbinxu/datasets/mnist/mnist_colorback.npz", train_ratio=0.8, fold_k=None)
# # colornum_mnist = datamanager_mnist(datapath="/home/scw4750/songbinxu/datasets/mnist/mnist_colornum.npz", train_ratio=0.8, fold_k=None)

# a = mnist(64, phase='test', var_list=['data'])
# tmp = [[] for _ in range(5)]
# for i in range(5):
#     for j in range(5):
#         tmp[i].append(a['data'][i*5+j])
#     tmp[i] = np.concatenate(tmp[i], 1)
# tmp = np.concatenate(tmp, 0)
# print tmp.shape
# plt.imshow(tmp[:,:,0], cmap=plt.cm.gray)
# plt.savefig('figs/cyclegan/ori.png')
